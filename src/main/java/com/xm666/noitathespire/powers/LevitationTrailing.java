...
import com.xm666.noitathespire.cards.Levitate;
import com.xm666.noitathespire.cards.TrailBeam;
import com.xm666.noitathespire.util.ModUtil;

public class LevitationTrailing extends AbstractPower {
    ...
    public LevitationTrailing(AbstractCreature owner, int amount) {
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
    ...
    public void updateDescription() {
        String upgrade = amount > 1 ? "+" + (amount - 1) : "";
        this.description = String.format(DESCRIPTIONS[0], upgrade);
    }

    @Override
    public void onUseCard(AbstractCard card, UseCardAction action) {
        super.onUseCard(card, action);
        if (card instanceof Levitate) {
            TrailBeam beam = new TrailBeam();
